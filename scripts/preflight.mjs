...
const files = ['.codex-plugin/plugin.json', '.mcp.json', 'agents/helioterm-mcp.toml', 'skills/helioterm/SKILL.md', 'scripts/firewall.mjs', 'scripts/inspect-proof.mjs', 'scripts/find-rollout.mjs', 'scripts/install-project.mjs', 'scripts/mcp-server.mjs'];
const checks = [
  ...
  { name: 'leaf-invariant', pass: role.includes('delegate') && role.includes('persistent semantic terminal') },
  { name: 'deterministic-map', pass: role.includes('test runs exactly `node --test <argument>`') && role.includes('Never read package metadata') },
];
